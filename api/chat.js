...
// ════════════════════════════════════════════════════════════════════════
//  KAMI MUSIC P2P API — Redis Backend
//  Được thêm vào để hỗ trợ ứng dụng nghe nhạc ngang hàng
// ════════════════════════════════════════════════════════════════════════

const MUSIC_REDIS_PREFIX = 'kami:music';
const MUSIC_TTL_DAYS = 365;

async function getAllSongs() {
  if (!redis) {
    const songs = [];
    for (const [key, item] of memoryStore.entries()) {
      if (key.startsWith(`${MUSIC_REDIS_PREFIX}:song:`)) {
        const song = item.value;
        if (song && (!item.expires || Date.now() < item.expires)) {
          songs.push(song);
        }
      }
    }
  ...
  try {
    ...
    let cursor = 0;
    do {
      const result = await redis.scan(cursor, { match: `${MUSIC_REDIS_PREFIX}:song:*`, count: 100 });
      cursor = result[0];
      keys.push(...result[1]);
    } while (cursor !== 0);
...
async function getSongById(songId) {
  const key = `${MUSIC_REDIS_PREFIX}:song:${songId}`;
  if (!redis) {
    const item = memoryStore.get(key);
    if (!item) return null;
    if (item.expires && Date.now() > item.expires) {
  ...
  try {
    ...
    console.error('Redis getSongById error:', error);
    return null;
  }
}

async function saveSong(song) {
  const key = `${MUSIC_REDIS_PREFIX}:song:${song.id || song.file_id || song.message_id}`;
  const ttl = MUSIC_TTL_DAYS * 86400;
  if (!redis) {
    memoryStore.set(key, { value: song, expires: Date.now() + ttl * 1000 });
    return true;
  }
  try {
    await redis.set(key, JSON.stringify(song), { ex: ttl });
    return true;
  } catch (error) {
...
async function getMusicStats() {
  ...
  const uniqueUsers = new Set(songs.map(s => s.userId).filter(Boolean));
  return {
...
async function handleGetSongs(req, res) {
  try {
    const { limit = 9999, offset = 0, sort = 'newest', q } = req.query || {};
    ...
    const end = start + (parseInt(limit) || 9999);
    ...
    console.error('handleGetSongs error:', error);
...
async function handleUpload(req, res) {
  try {
    const { file_id, file_name, file_size, message_id, userId, date, file_url } = req.body;

    if (!file_id || !file_name) {
      return res.status(400).json({ success: false, error: 'Thiếu file_id hoặc file_name' });
    }
    ...
    const existing = await getSongById(file_id);
    if (existing) {
      return res.status(200).json({ success: true, ok: true, message: 'File đã tồn tại', song: existing });
    }
    ...
    const song = {
      ...
      message_id: parseInt(message_id) || 0,
      userId: userId,
    ...
    if (!saved) {
      return res.status(500).json({ success: false, error: 'Không thể lưu vào Redis' });
    }
...
async function handleDelete(req, res) {
  try {
    ...
    if (!message_id) {
      return res.status(400).json({ success: false, error: 'Thiếu message_id' });
    }

    const song = await getSongById(message_id);
    if (!song) {
    ...
    const deleted = await deleteSong(message_id);
    if (!deleted) {
      return res.status(500).json({ success: false, error: 'Không thể xóa' });
    }

    return res.status(200).json({ success: true, ok: true, message: 'Đã xóa' });
  } catch (error) {
...
async function handleSearch(req, res) {
  try {
    ...
    songs = songs.filter(s => {
      ...
      const uploader = (s.userId || '').toLowerCase();
      return name.includes(query) || uploader.includes(query);
    });

    songs.sort((a, b) => (b.date || 0) - (a.date || 0));

    const results = songs.slice(0, parseInt(limit) || 100);
...
export default async function handler(req, res) {
  ...
  // ════════════════════════════════════════════════════════════════════════
  //  KAMI MUSIC P2P ROUTE DISPATCHER
  // ════════════════════════════════════════════════════════════════════════
  const url = req.url || '';

  if (req.method === 'GET' && (url === '/api/songs' || url.startsWith('/api/songs?'))) {
  ...
  if (req.method === 'GET' && (url === '/api/search' || url.startsWith('/api/search?'))) {
