...
// FIXED: Thread-safe cache với auto-cleanup
...
// Cache cho AI detection (giảm overhead)
const detectionCache = new NodeCache({
  stdTTL: 1800,          // 30 phút
  maxKeys: 200
});
...
function quickKeywordCheck(message) {
  ...
  const neverSearch = [
    ...
    'ok', 'được', 'rồi', 'ừ', 'uhm',
    'bạn khỏe không', 'thế nào',
    'bạn là ai', 'tên bạn'
  ];
  
  // Check never search first
...
// FIXED: Cải thiện heuristic analysis
...
// ============ SMART SEARCH với Parallel Strategy ============
...
async function smartSearch(query, searchType, groq) {
  ...
  try {
    // Strategy 1: Parallel search cho tốc độ
    if (searchType === 'realtime' && SERPER_API_KEY) {
      // Realtime cần Google search
    ...
    if (searchType === 'knowledge') {
      // Knowledge queries: Wikipedia + Tavily parallel
      const searches = [
